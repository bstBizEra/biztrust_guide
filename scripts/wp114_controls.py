...
need or drift before moving anything. `check_source_main_line` below is deliberately NOT among
them: its docstring is this package's own recorded measurement, wp112's says something different
...
from control_harness import (HOLE_DECLARED, VERDICT, arguments, clone, edit, git_out,
                             report_unmutated, run_script, run_suite, suite_controls, summarise)
...
def main() -> int:
    ...
    return summarise(len(SUITE_CONTROLS) + len(SCRIPT_CONTROLS) + 1, bad)
