...
const progress=$('#readingProgress');if(progress)addEventListener('scroll',()=>{const d=document.documentElement;const p=d.scrollTop/(d.scrollHeight-d.clientHeight)*100;progress.style.width=Math.min(100,p)+'%';},{passive:true});
...
const overlay=$('#searchOverlay'),input=$('#searchInput'),results=$('#searchResults'),searchButton=$('#searchButton');
function openSearch(){if(!overlay)return;overlay.hidden=false;input.value='';results.innerHTML='<p>Start typing to search the implementation guide.</p>';setTimeout(()=>input.focus(),20)}function closeSearch(){if(overlay)overlay.hidden=true}
