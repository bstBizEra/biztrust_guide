...
3. Confirm every tracked `*.html` reaches the publishing artifact — CI step *Verify every tracked page reached the artifact* — not only root `index.html`. Nineteen of the twenty tracked pages live under `stages/`, `phases/`, `reference/` and `landing/`.
