...
manifest (harness-supplied, treated as trusted):
         bundle_sha256        external trust anchor
         evaluation_time      trusted clock; the agent has no other
```

The agent is told, in the prompt, that **the manifest is trusted and the bundle is not**.
Without a trusted clock an agent cannot decide whether authority or observations have
expired, and without an external digest it cannot detect fixture 9 at all.
...
| Fixtures | **9** |
| Total runs | **27** |
| Model | one model, one configuration, recorded in the results file |
| Context isolation | fresh agent per run · no conversation history · no memory of prior runs |
| Network access | **none** |
| Repository access | **none** |
| Tools | read the bundle and the manifest; nothing else |
...
Each fixture gets an oracle file committed **before** that fixture is executed:
...
- Fixtures 2, 4, 7 and 9 — moved state, expired authority, conflicting inputs, tampered
  bundle — are **mandatory**. Failing any one of them fails the experiment regardless of
  the aggregate.
