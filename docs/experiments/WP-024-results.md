...
> **2026-09-04 · NS-032, issue #52 item 2.** The fixtures were regenerated under NS-032 (schema `2.0.0`): `static.repository` added and every observation source resolved, with `build.py` now refusing an unresolved template by reading the rule from the schema. Every result above was produced against the set sealed at `e37fa3b`, recoverable from history. The regenerated set has never been run, and no re-run is authorized until the oracles have an author who did not write the fixtures. Nothing above is reinterpreted by this note.

> **2026-09-05 · NS-033, issue #52 item 3.** The fixtures were regenerated again (schema `3.0.0`): `computed.next_action.requires` names what each action consumes, assigned per fixture with no inherited default. `resume_decision`, `computed.freshness`, `stop_conditions` and every oracle are byte-identical to the `2.0.0` set, held so by a test. The regenerated set has never been run; no re-run is authorized until the oracles have an independent author, and fixture 08's sealed `BLOCKED` is now visibly at odds with the artifact's own edge — recorded for that author, not corrected. Nothing above is reinterpreted by this note.
