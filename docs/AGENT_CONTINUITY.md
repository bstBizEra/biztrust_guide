...
> **Corrected 2026-09-04 (issue #66).** Three claims in the note above have since changed; the branch-protection claim has not. Measured on 2026-09-04: `GET /pages` reports `build_type: workflow`; the `github-pages` environment carries two branch policies, `main` and the earlier pattern `None`; and the environment has recorded a successful deployment for every push to `main` since 2026-09-03T12:51:36Z (deployment 6244308440), so the control *has* run. `GET /branches/main/protection` still returns 404, so items 1, 2, 3, 4, 6 and 7 remain target state. Item 5 is in force only by configuration: the deploy job is bound to the environment and the environment admits `main` by name, but nothing protects `main` itself, so the restriction guards the deploy path, not the branch. The note above is left as written because it was true when dated; this correction moves no safety statement in the permissive direction that it cannot support with a measurement.
