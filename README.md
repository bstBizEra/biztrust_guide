...
| `tests/test_checkpoints_match_schema.py` | Validates every committed checkpoint against `schemas/session-checkpoint.schema.json` with a stdlib checker, and fails if either schema uses a keyword the checker does not implement — so a schema edit cannot silently go unenforced. Three checkpoints that predate the check are registered as a ratchet that may only shrink. |
| `.gitattributes` | Pins LF on every text file. The sealed-fixture checks compare bytes, so a checkout that rewrote LF to CRLF failed two tests on a clean tree before this existed. |
